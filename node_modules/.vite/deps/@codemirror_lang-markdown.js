...
var Option = class {
  constructor(completion, source, match, score3) {
...
var FuzzyMatcher = class {
  ...
  match(word) {
    ...
    if (chars.length == 1) {
      ...
      let score3 = firstSize == word.length ? 0 : -100;
      if (first == chars[0]) ;
      else if (first == folded[0])
        score3 += -200;
      else
        return null;
      return this.ret(score3, [0, firstSize]);
    }
  ...
  result(score3, positions, word) {
    ...
    return this.ret(score3 - word.length, result);
  }
};
...
var CompletionTooltip = class {
  constructor(view, stateField, applyCompletion3) {
    this.view = view;
    this.stateField = stateField;
    this.applyCompletion = applyCompletion3;
    this.info = null;
...
function completionTooltip(stateField, applyCompletion3) {
  return (view) => new CompletionTooltip(view, stateField, applyCompletion3);
}
...
function sortOptions(active, state) {
  ...
  for (let opt of options.sort((a, b) => b.score - a.score || compare(a.completion, b.completion))) {
    let cur3 = opt.completion;
    if (!prev || prev.label != cur3.label || prev.detail != cur3.detail || prev.type != null && cur3.type != null && prev.type != cur3.type || prev.apply != cur3.apply || prev.boost != cur3.boost)
      result.push(opt);
...
var FragmentCursor = class {
  ...
  takeNodes(cx) {
    let cur3 = this.cursor, off = this.fragment.offset, fragEnd = this.fragmentEnd - (this.fragment.openEnd ? 1 : 0);
    let start = cx.absoluteLineStart, end = start, blockI = cx.block.children.length;
    let prevEnd = end, prevI = blockI;
    for (; ; ) {
      if (cur3.to - off > fragEnd) {
        if (cur3.type.isAnonymous && cur3.firstChild())
          continue;
        break;
      }
      let pos = toRelative(cur3.from - off, cx.ranges);
      if (cur3.to - off <= cx.ranges[cx.rangeI].to) {
        cx.addNode(cur3.tree, pos);
      } else {
        let dummy = new Tree(cx.parser.nodeSet.types[Type.Paragraph], [], [], 0, cx.block.hashProp);
        cx.reusePlaceholders.set(dummy, cur3.tree);
        cx.addNode(dummy, pos);
      }
      if (cur3.type.is("Block")) {
        if (NotLast.indexOf(cur3.type.id) < 0) {
          end = cur3.to - off;
...
var Stack = class _Stack {
  ...
  @internal
  */
  constructor(p, stack, state, reducePos, pos, score3, buffer, bufferBase, curContext, lookAhead = 0, parent) {
    this.p = p;
    this.stack = stack;
    this.state = state;
  ...
  */
  storeNode(term, start, end, size = 4, mustSink = false) {
    if (term == 0 && (!this.stack.length || this.stack[this.stack.length - 1] < this.buffer.length + this.bufferBase)) {
      let cur3 = this, top = this.buffer.length;
      if (top == 0 && cur3.parent) {
        top = cur3.bufferBase - cur3.parent.bufferBase;
        cur3 = cur3.parent;
      }
      if (top > 0 && cur3.buffer[top - 4] == 0 && cur3.buffer[top - 1] > -1) {
        if (start == end)
          return;
        if (cur3.buffer[top - 2] >= start) {
          cur3.buffer[top - 2] = end;
          return;
        }
      }
    }
  ...
  configure(config3) {
    ...
    if (config3.tokenizers)
      copy.tokenizers = this.tokenizers.map((t) => {
        let found = config3.tokenizers.find((r) => r.from == t);
...
var pickedCompletion2 = Annotation.define();
function insertCompletionText2(state, text, from, to) {
  let { main } = state.selection, fromOff = from - main.from, toOff = to - main.from;
  return Object.assign(Object.assign({}, state.changeByRange((range) => {
    if (range != main && from != to && state.sliceDoc(range.from + fromOff, range.from + toOff) != state.sliceDoc(from, to))
      return { range };
...
var SourceCache2 = /* @__PURE__ */ new WeakMap();
function asSource2(source) {
  if (!Array.isArray(source))
    return source;
  let known = SourceCache2.get(source);
  if (!known)
    SourceCache2.set(source, known = completeFromList2(source));
  return known;
}
var startCompletionEffect2 = StateEffect.define();
var closeCompletionEffect2 = StateEffect.define();
var FuzzyMatcher2 = class {
  constructor(pattern) {
    this.pattern = pattern;
    this.chars = [];
    this.folded = [];
...
var completionConfig2 = Facet.define({
  combine(configs) {
    return combineConfig(configs, {
      activateOnTyping: true,
      activateOnCompletion: () => false,
      ...
      icons: true,
      addToOptions: [],
      positionInfo: defaultPositionInfo2,
      filterStrict: false,
      compareCompletions: (a, b) => a.label.localeCompare(b.label),
      interactionDelay: 75,
      updateSyncTime: 100
      ...
      closeOnBlur: (a, b) => a && b,
      icons: (a, b) => a && b,
      tooltipClass: (a, b) => (c) => joinClass2(a(c), b(c)),
      optionClass: (a, b) => (c) => joinClass2(a(c), b(c)),
      addToOptions: (a, b) => a.concat(b),
      filterStrict: (a, b) => a || b
    });
  }
});
function joinClass2(a, b) {
  return a ? b ? a + " " + b : a : b;
}
function defaultPositionInfo2(view, list, option, info, space4, tooltip) {
  let rtl = view.textDirection == Direction.RTL, left = rtl, narrow = false;
  let side = "top", offset, maxWidth;
  let spaceLeft = list.left - space4.left, spaceRight = space4.right - list.right;
  let infoWidth = info.right - info.left, infoHeight = info.bottom - info.top;
...
function optionContent2(config3) {
  let content = config3.addToOptions.slice();
  if (config3.icons)
    content.push({
      render(completion) {
...
function rangeAroundSelected2(total, selected, max) {
  if (total <= max)
    return { from: 0, to: total };
  if (selected < 0)
    selected = 0;
...
var CompletionTooltip2 = class {
  constructor(view, stateField, applyCompletion3) {
    ...
    let cState = view.state.field(stateField);
    let { options, selected } = cState.open;
    let config3 = view.state.facet(completionConfig2);
    this.optionContent = optionContent2(config3);
    this.optionClass = config3.optionClass;
    this.tooltipClass = config3.tooltipClass;
    this.range = rangeAroundSelected2(options.length, selected, config3.maxRenderedOptions);
    this.dom = document.createElement("div");
    this.dom.className = "cm-tooltip-autocomplete";
    this.updateTooltipClass(view.state);
    this.dom.addEventListener("mousedown", (e) => {
    ...
    this.dom.addEventListener("focusout", (e) => {
      let state = view.state.field(this.stateField, false);
      if (state && state.tooltip && view.state.facet(completionConfig2).closeOnBlur && e.relatedTarget != view.contentDOM)
        view.dispatch({ effects: closeCompletionEffect2.of(null) });
    });
    this.showOptions(options, cState.id);
  }
  mount() {
  ...
  update(update) {
    ...
    if (cState != prevState) {
      let { options, selected, disabled } = cState.open;
      if (!prevState.open || prevState.open.options != options) {
        this.range = rangeAroundSelected2(options.length, selected, update.state.facet(completionConfig2).maxRenderedOptions);
        this.showOptions(options, cState.id);
      }
      this.updateSel();
      if (disabled != ((_a = prevState.open) === null || _a === void 0 ? void 0 : _a.disabled))
  ...
  updateSel() {
    let cState = this.view.state.field(this.stateField), open = cState.open;
    if (open.selected > -1 && open.selected < this.range.from || open.selected >= this.range.to) {
      this.range = rangeAroundSelected2(open.options.length, open.selected, this.view.state.facet(completionConfig2).maxRenderedOptions);
      this.showOptions(open.options, cState.id);
    }
    if (this.updateSelectedOption(open.selected)) {
      this.destroyInfo();
  ...
  measureInfo() {
    ...
    if (selRect.top > Math.min(space4.bottom, listRect.bottom) - 10 || selRect.bottom < Math.max(space4.top, listRect.top) + 10)
      return null;
    return this.view.state.facet(completionConfig2).positionInfo(this.view, listRect, selRect, infoRect, space4, this.dom);
  }
  placeInfo(pos) {
    if (this.info) {
      if (pos) {
...
function completionTooltip2(stateField, applyCompletion3) {
  return (view) => new CompletionTooltip2(view, stateField, applyCompletion3);
}
...
function score2(option) {
  return (option.boost || 0) * 100 + (option.apply ? 10 : 0) + (option.info ? 5 : 0) + (option.type ? 1 : 0);
}
function sortOptions2(active, state) {
  let options = [];
  let sections = null;
  let addOption = (option) => {
    options.push(option);
  ...
  let conf = state.facet(completionConfig2);
  for (let a of active)
    if (a.hasResult()) {
      let getMatch = a.result.getMatch;
      if (a.result.filter === false) {
        ...
        let pattern = state.sliceDoc(a.from, a.to), match;
        let matcher = conf.filterStrict ? new StrictMatcher2(pattern) : new FuzzyMatcher2(pattern);
        for (let option of a.result.options)
          if (match = matcher.match(option.label)) {
            let matched = !option.displayLabel ? match.matched : getMatch ? getMatch(option, match.matched) : [];
  ...
  for (let opt of options.sort((a, b) => b.score - a.score || compare(a.completion, b.completion))) {
    ...
    if (!prev || prev.label != cur3.label || prev.detail != cur3.detail || prev.type != null && cur3.type != null && prev.type != cur3.type || prev.apply != cur3.apply || prev.boost != cur3.boost)
      result.push(opt);
    else if (score2(opt.completion) > score2(prev))
      result[result.length - 1] = opt;
    prev = opt.completion;
  }
  return result;
}
var CompletionDialog2 = class _CompletionDialog {
  constructor(options, attrs, tooltip, timestamp, selected, disabled) {
    this.options = options;
    this.attrs = attrs;
    this.tooltip = tooltip;
  ...
  setSelected(selected, id2) {
    return selected == this.selected || selected >= this.options.length ? this : new _CompletionDialog(this.options, makeAttrs2(id2, selected), this.tooltip, this.timestamp, selected, this.disabled);
  }
  static build(active, state, id2, prev, conf, didSetActive) {
    if (prev && !didSetActive && active.some((s) => s.isPending))
      return prev.setDisabled();
    let options = sortOptions2(active, state);
    if (!options.length)
      return prev && active.some((a) => a.isPending) ? prev.setDisabled() : null;
    let selected = state.facet(completionConfig2).selectOnOpen ? 0 : -1;
    if (prev && prev.selected != selected && prev.selected != -1) {
      let selectedValue = prev.options[prev.selected].completion;
      for (let i = 0; i < options.length; i++)
        if (options[i].completion == selectedValue) {
    ...
    return new _CompletionDialog(options, makeAttrs2(id2, selected), {
      pos: active.reduce((a, b) => b.hasResult() ? Math.min(a, b.from) : a, 1e8),
      create: createTooltip2,
      above: conf.aboveCursor
    }, prev ? prev.timestamp : Date.now(), selected, false);
  }
  map(changes) {
...
var CompletionState2 = class _CompletionState {
  constructor(active, id2, open) {
    this.active = active;
    this.id = id2;
    this.open = open;
  ...
  update(tr) {
    let { state } = tr, conf = state.facet(completionConfig2);
    let sources = conf.override || state.languageDataAt("autocomplete", cur2(state)).map(asSource2);
    let active = sources.map((source) => {
      let value = this.active.find((s) => s.source == source) || new ActiveSource2(
        source,
        this.active.some(
          (a) => a.state != 0
          /* State.Inactive */
    ...
    if (active.length == this.active.length && active.every((a, i) => a == this.active[i]))
      active = this.active;
    let open = this.open, didSet = tr.effects.some((e) => e.is(setActiveEffect2));
    if (open && tr.docChanged)
      open = open.map(tr.changes);
    if (tr.selection || active.some((a) => a.hasResult() && tr.changes.touchesRange(a.from, a.to)) || !sameResults2(active, this.active) || didSet)
      open = CompletionDialog2.build(active, state, this.id, open, conf, didSet);
    else if (open && open.disabled && !active.some((a) => a.isPending))
      open = null;
    if (!open && active.every((a) => !a.isPending) && active.some((a) => a.hasResult()))
      active = active.map((a) => a.hasResult() ? new ActiveSource2(
        a.source,
        0
        /* State.Inactive */
      ) : a);
    for (let effect of tr.effects)
      if (effect.is(setSelectedEffect2))
        open = open && open.setSelected(effect.value, this.id);
    return active == this.active && open == this.open ? this : new _CompletionState(active, this.id, open);
  }
  get tooltip() {
  ...
  get attrs() {
    return this.open ? this.open.attrs : this.active.length ? baseAttrs2 : noAttrs2;
  }
};
function sameResults2(a, b) {
  if (a == b)
    return true;
  for (let iA = 0, iB = 0; ; ) {
    while (iA < a.length && !a[iA].hasResult())
...
var noAttrs2 = {};
function makeAttrs2(id2, selected) {
  let result = {
    "aria-autocomplete": "list",
    "aria-haspopup": "listbox",
    "aria-controls": id2
...
var none3 = [];
function getUpdateType2(tr, conf) {
  if (tr.isUserEvent("input.complete")) {
    let completion = tr.annotation(pickedCompletion2);
    if (completion && conf.activateOnCompletion(completion))
      return 4 | 8;
  }
  let typing = tr.isUserEvent("input.type");
...
var ActiveSource2 = class _ActiveSource {
  constructor(source, state, explicit = false) {
    this.source = source;
    this.state = state;
    this.explicit = explicit;
  ...
  update(tr, conf) {
    let type = getUpdateType2(tr, conf), value = this;
    if (type & 8 || type & 16 && this.touches(tr))
      value = new _ActiveSource(
        value.source,
        0
    ...
    value = value.updateFor(tr, type);
    for (let effect of tr.effects) {
      if (effect.is(startCompletionEffect2))
        value = new _ActiveSource(value.source, 1, effect.value);
      else if (effect.is(closeCompletionEffect2))
        value = new _ActiveSource(
          value.source,
          0
          /* State.Inactive */
        );
      else if (effect.is(setActiveEffect2)) {
        for (let active of effect.value)
          if (active.source == value.source)
            value = active;
      }
  ...
  touches(tr) {
    return tr.changes.touchesRange(cur2(tr.state));
  }
};
var ActiveResult2 = class _ActiveResult extends ActiveSource2 {
  constructor(source, explicit, limit, result, from, to) {
    super(source, 3, explicit);
    this.limit = limit;
    this.result = result;
  ...
  updateFor(tr, type) {
    ...
    if (result.map && !tr.changes.empty)
      result = result.map(result, tr.changes);
    let from = tr.changes.mapPos(this.from), to = tr.changes.mapPos(this.to, 1);
    let pos = cur2(tr.state);
    if (pos > to || !result || type & 2 && (cur2(tr.startState) == this.from || pos < this.limit))
      return new ActiveSource2(
        this.source,
        type & 4 ? 1 : 0
        /* State.Inactive */
      );
    let limit = tr.changes.mapPos(this.limit);
    if (checkValid2(result.validFor, tr.state, from, to))
      return new _ActiveResult(this.source, this.explicit, limit, result, from, to);
    if (result.update && (result = result.update(result, from, to, new CompletionContext2(tr.state, pos, false))))
      return new _ActiveResult(this.source, this.explicit, limit, result, result.from, (_a = result.to) !== null && _a !== void 0 ? _a : cur2(tr.state));
    return new ActiveSource2(this.source, 1, this.explicit);
  }
  map(mapping) {
    if (mapping.empty)
      return this;
...
function checkValid2(validFor, state, from, to) {
  if (!validFor)
    return false;
  let text = state.sliceDoc(from, to);
  return typeof validFor == "function" ? validFor(text, from, to, state) : ensureAnchor2(validFor, true).test(text);
}
var setActiveEffect2 = StateEffect.define({
  map(sources, mapping) {
    return sources.map((s) => s.map(mapping));
  }
});
var setSelectedEffect2 = StateEffect.define();
var completionState2 = StateField.define({
  create() {
    return CompletionState2.start();
  },
  update(value, tr) {
    return value.update(tr);
  },
...
function applyCompletion2(view, option) {
  const apply = option.completion.apply || option.completion.label;
  let result = view.state.field(completionState2).active.find((a) => a.source == option.source);
  if (!(result instanceof ActiveResult2))
    return false;
  if (typeof apply == "string")
    view.dispatch(Object.assign(Object.assign({}, insertCompletionText2(view.state, apply, result.from, result.to)), { annotations: pickedCompletion2.of(option.completion) }));
  else
    apply(view, option.completion, result.from, result.to);
  return true;
}
var createTooltip2 = completionTooltip2(completionState2, applyCompletion2);
function moveCompletionSelection2(forward, by = "option") {
  return (view) => {
    let cState = view.state.field(completionState2, false);
    if (!cState || !cState.open || cState.open.disabled || Date.now() - cState.open.timestamp < view.state.facet(completionConfig2).interactionDelay)
      return false;
    let step = 1, tooltip;
    if (by == "page" && (tooltip = getTooltip(view, cState.open.tooltip)))
      step = Math.max(2, Math.floor(tooltip.dom.offsetHeight / tooltip.dom.querySelector("li").offsetHeight) - 1);
    ...
    if (selected < 0)
      ...
      selected = by == "page" ? length - 1 : 0;
    view.dispatch({ effects: setSelectedEffect2.of(selected) });
    return true;
  };
}
var acceptCompletion2 = (view) => {
  let cState = view.state.field(completionState2, false);
  if (view.state.readOnly || !cState || !cState.open || cState.open.selected < 0 || cState.open.disabled || Date.now() - cState.open.timestamp < view.state.facet(completionConfig2).interactionDelay)
    return false;
  return applyCompletion2(view, cState.open.options[cState.open.selected]);
};
var startCompletion2 = (view) => {
  let cState = view.state.field(completionState2, false);
  if (!cState)
    return false;
  view.dispatch({ effects: startCompletionEffect2.of(true) });
  return true;
};
var closeCompletion2 = (view) => {
  let cState = view.state.field(completionState2, false);
  if (!cState || !cState.active.some(
    (a) => a.state != 0
    /* State.Inactive */
...
var MaxUpdateCount2 = 50;
var MinAbortTime2 = 1e3;
var completionPlugin2 = ViewPlugin.fromClass(class {
  constructor(view) {
    this.view = view;
    this.debounceUpdate = -1;
    this.running = [];
    ...
    this.pendingStart = false;
    this.composing = 0;
    for (let active of view.state.field(completionState2).active)
      if (active.isPending)
        this.startQuery(active);
  }
  update(update) {
    let cState = update.state.field(completionState2);
    let conf = update.state.facet(completionConfig2);
    if (!update.selectionSet && !update.docChanged && update.startState.field(completionState2) == cState)
      return;
    let doesReset = update.transactions.some((tr) => {
      let type = getUpdateType2(tr, conf);
      return type & 8 || (tr.selection || tr.docChanged) && !(type & 3);
    });
    for (let i = 0; i < this.running.length; i++) {
      let query = this.running[i];
      if (doesReset || query.context.abortOnDocChange && update.docChanged || query.updates.length + update.transactions.length > MaxUpdateCount2 && Date.now() - query.time > MinAbortTime2) {
        for (let handler of query.context.abortListeners) {
          try {
            handler();
          } catch (e) {
    ...
    if (this.debounceUpdate > -1)
      clearTimeout(this.debounceUpdate);
    if (update.transactions.some((tr) => tr.effects.some((e) => e.is(startCompletionEffect2))))
      this.pendingStart = true;
    let delay = this.pendingStart ? 50 : conf.activateOnTypingDelay;
    this.debounceUpdate = cState.active.some((a) => a.isPending && !this.running.some((q) => q.active.source == a.source)) ? setTimeout(() => this.startUpdate(), delay) : -1;
    if (this.composing != 0)
  ...
  startUpdate() {
    this.debounceUpdate = -1;
    this.pendingStart = false;
    let { state } = this.view, cState = state.field(completionState2);
    for (let active of cState.active) {
      if (active.isPending && !this.running.some((r) => r.active.source == active.source))
        this.startQuery(active);
    }
    if (this.running.length && cState.open && cState.open.disabled)
      this.debounceAccept = setTimeout(() => this.accept(), this.view.state.facet(completionConfig2).updateSyncTime);
  }
  startQuery(active) {
    let { state } = this.view, pos = cur2(state);
    let context = new CompletionContext2(state, pos, active.explicit, this.view);
    let pending = new RunningQuery2(active, context);
    this.running.push(pending);
    Promise.resolve(active.source(context)).then((result) => {
      if (!pending.context.aborted) {
        pending.done = result || null;
      ...
      this.view.dispatch({ effects: closeCompletionEffect2.of(null) });
      logException(this.view.state, err);
    });
  }
  scheduleAccept() {
  ...
  accept() {
    ...
    this.debounceAccept = -1;
    let updated = [];
    let conf = this.view.state.facet(completionConfig2), cState = this.view.state.field(completionState2);
    for (let i = 0; i < this.running.length; i++) {
      let query = this.running[i];
      if (query.done === void 0)
        continue;
      this.running.splice(i--, 1);
      if (query.done) {
        let pos = cur2(query.updates.length ? query.updates[0].startState : this.view.state);
        let limit = Math.min(pos, query.done.from + (query.active.explicit ? 0 : 1));
        let active = new ActiveResult2(query.active.source, query.active.explicit, limit, query.done, query.done.from, (_a = query.done.to) !== null && _a !== void 0 ? _a : pos);
        for (let tr of query.updates)
          active = active.update(tr, conf);
        if (active.hasResult()) {
          updated.push(active);
    ...
    if (updated.length || cState.open && cState.open.disabled)
      this.view.dispatch({ effects: setActiveEffect2.of(updated) });
  }
}, {
  eventHandlers: {
    blur(event) {
      let state = this.view.state.field(completionState2, false);
      if (state && state.tooltip && this.view.state.facet(completionConfig2).closeOnBlur) {
        let dialog = state.open && getTooltip(this.view, state.open.tooltip);
        if (!dialog || !dialog.dom.contains(event.relatedTarget))
          setTimeout(() => this.view.dispatch({ effects: closeCompletionEffect2.of(null) }), 10);
      }
    },
    compositionstart() {
      this.composing = 1;
    ...
    compositionend() {
      if (this.composing == 3) {
        setTimeout(() => this.view.dispatch({ effects: startCompletionEffect2.of(false) }), 20);
      }
      this.composing = 0;
    }
  }
});
var windows2 = typeof navigator == "object" && /Win/.test(navigator.platform);
var commitCharacters2 = Prec.highest(EditorView.domEventHandlers({
  keydown(event, view) {
    let field = view.state.field(completionState2, false);
    if (!field || !field.open || field.open.disabled || field.open.selected < 0 || event.key.length > 1 || event.ctrlKey && !(windows2 && event.altKey) || event.metaKey)
      return false;
    let option = field.open.options[field.open.selected];
    let result = field.active.find((a) => a.source == option.source);
    let commitChars = option.completion.commitCharacters || result.result.commitCharacters;
    if (commitChars && commitChars.indexOf(event.key) > -1)
      applyCompletion2(view, option);
    return false;
  }
}));
...
var fieldMarker2 = Decoration.widget({ widget: new class extends WidgetType {
  toDOM() {
    let span = document.createElement("span");
    span.className = "cm-snippetFieldPosition";
    return span;
...
var fieldRange2 = Decoration.mark({ class: "cm-snippetField" });
var ActiveSnippet2 = class _ActiveSnippet {
  constructor(ranges, active) {
    this.ranges = ranges;
    this.active = active;
    this.deco = Decoration.set(ranges.map((r) => (r.from == r.to ? fieldMarker2 : fieldRange2).range(r.from, r.to)));
  }
  map(changes) {
    let ranges = [];
    for (let r of this.ranges) {
...
var setActive2 = StateEffect.define({
  map(value, changes) {
    return value && value.map(changes);
  }
});
var moveToField2 = StateEffect.define();
var snippetState2 = StateField.define({
  create() {
    return null;
  },
  update(value, tr) {
    for (let effect of tr.effects) {
      if (effect.is(setActive2))
        return effect.value;
      if (effect.is(moveToField2) && value)
        return new ActiveSnippet2(value.ranges, effect.value);
    }
    if (value && tr.docChanged)
      value = value.map(tr.changes);
    if (value && tr.selection && !value.selectionInsideField(tr.selection))
...
function fieldSelection2(ranges, field) {
  return EditorSelection.create(ranges.filter((r) => r.field == field).map((r) => EditorSelection.range(r.from, r.to)));
}
function snippet(template) {
  ...
  return (editor, completion, from, to) => {
    ...
    if (ranges.length)
      spec.selection = fieldSelection2(ranges, 0);
    if (ranges.some((r) => r.field > 0)) {
      let active = new ActiveSnippet2(ranges, 0);
      let effects = spec.effects = [setActive2.of(active)];
      if (editor.state.field(snippetState2, false) === void 0)
        effects.push(StateEffect.appendConfig.of([snippetState2, addSnippetKeymap2, snippetPointerHandler2, baseTheme2]));
    }
    editor.dispatch(editor.state.update(spec));
  };
}
function moveField2(dir) {
  return ({ state, dispatch }) => {
    let active = state.field(snippetState2, false);
    if (!active || dir < 0 && active.active == 0)
      return false;
    let next = active.active + dir, last = dir > 0 && !active.ranges.some((r) => r.field == next + dir);
    dispatch(state.update({
      selection: fieldSelection2(active.ranges, next),
      effects: setActive2.of(last ? null : new ActiveSnippet2(active.ranges, next)),
      scrollIntoView: true
    }));
    return true;
  };
}
var clearSnippet2 = ({ state, dispatch }) => {
  let active = state.field(snippetState2, false);
  if (!active)
    return false;
  dispatch(state.update({ effects: setActive2.of(null) }));
  return true;
};
var nextSnippetField2 = moveField2(1);
var prevSnippetField2 = moveField2(-1);
var defaultSnippetKeymap2 = [
  { key: "Tab", run: nextSnippetField2, shift: prevSnippetField2 },
  { key: "Escape", run: clearSnippet2 }
];
var snippetKeymap2 = Facet.define({
  combine(maps) {
    return maps.length ? maps[0] : defaultSnippetKeymap2;
  }
});
var addSnippetKeymap2 = Prec.highest(keymap.compute([snippetKeymap2], (state) => state.facet(snippetKeymap2)));
function snippetCompletion(template, completion) {
  return Object.assign(Object.assign({}, completion), { apply: snippet(template) });
}
var snippetPointerHandler2 = EditorView.domEventHandlers({
  mousedown(event, view) {
    let active = view.state.field(snippetState2, false), pos;
    if (!active || (pos = view.posAtCoords({ x: event.clientX, y: event.clientY })) == null)
      return false;
    let match = active.ranges.find((r) => r.from <= pos && r.to >= pos);
    if (!match || match.field == active.active)
      return false;
    view.dispatch({
      selection: fieldSelection2(active.ranges, match.field),
      effects: setActive2.of(active.ranges.some((r) => r.field > match.field) ? new ActiveSnippet2(active.ranges, match.field) : null),
      scrollIntoView: true
    });
    return true;
  }
...
var closeBracketEffect2 = StateEffect.define({
  map(value, mapping) {
    let mapped = mapping.mapPos(value, -1, MapMode.TrackAfter);
    return mapped == null ? void 0 : mapped;
  }
});
var closedBracket2 = new class extends RangeValue {
}();
closedBracket2.startSide = 1;
closedBracket2.endSide = -1;
var bracketState2 = StateField.define({
  create() {
    return RangeSet.empty;
  },
  update(value, tr) {
    ...
    for (let effect of tr.effects)
      if (effect.is(closeBracketEffect2))
        value = value.update({ add: [closedBracket2.range(effect.value, effect.value + 1)] });
    return value;
  }
});
var definedClosing2 = "()[]{}<>«»»«［］｛｝";
function closing2(ch) {
  for (let i = 0; i < definedClosing2.length; i += 2)
    if (definedClosing2.charCodeAt(i) == ch)
      return definedClosing2.charAt(i + 1);
  return fromCodePoint(ch < 128 ? ch : ch + 1);
}
...
var inputHandler2 = EditorView.inputHandler.of((view, from, to, insert) => {
  if ((android2 ? view.composing : view.compositionStarted) || view.state.readOnly)
    return false;
  let sel = view.state.selection.main;
  if (insert.length > 2 || insert.length == 2 && codePointSize(codePointAt(insert, 0)) == 1 || from != sel.from || to != sel.to)
    return false;
  let tr = insertBracket2(view.state, insert);
  if (!tr)
    return false;
  view.dispatch(tr);
  return true;
});
function insertBracket2(state, bracket) {
  let conf = config2(state, state.selection.main.head);
  let tokens = conf.brackets || defaults2.brackets;
  for (let tok of tokens) {
    let closed = closing2(codePointAt(tok, 0));
    if (bracket == tok)
      return closed == tok ? handleSame2(state, tok, tokens.indexOf(tok + tok + tok) > -1, conf) : handleOpen2(state, tok, closed, conf.before || defaults2.before);
    if (bracket == closed && closedBracketAt2(state, state.selection.main.from))
      return handleClose2(state, tok, closed);
  }
  return null;
}
function closedBracketAt2(state, pos) {
  let found = false;
  state.field(bracketState2).between(0, state.doc.length, (from) => {
    if (from == pos)
      found = true;
  });
  return found;
}
function nextChar2(doc, pos) {
  let next = doc.sliceString(pos, pos + 2);
  return next.slice(0, codePointSize(codePointAt(next, 0)));
}
function handleOpen2(state, open, close, closeBefore) {
  let dont = null, changes = state.changeByRange((range) => {
    if (!range.empty)
      return {
        changes: [{ insert: open, from: range.from }, { insert: close, from: range.to }],
        effects: closeBracketEffect2.of(range.to + open.length),
        range: EditorSelection.range(range.anchor + open.length, range.head + open.length)
      };
    let next = nextChar2(state.doc, range.head);
    if (!next || /\s/.test(next) || closeBefore.indexOf(next) > -1)
      return {
        changes: { insert: open + close, from: range.head },
        effects: closeBracketEffect2.of(range.head + open.length),
        range: EditorSelection.cursor(range.head + open.length)
      };
    return { range: dont = range };
  });
...
function handleClose2(state, _open, close) {
  let dont = null, changes = state.changeByRange((range) => {
    if (range.empty && nextChar2(state.doc, range.head) == close)
      return {
        changes: { from: range.head, to: range.head + close.length, insert: close },
        range: EditorSelection.cursor(range.head + close.length)
      };
...
function handleSame2(state, token, allowTriple, config3) {
  let stringPrefixes = config3.stringPrefixes || defaults2.stringPrefixes;
  let dont = null, changes = state.changeByRange((range) => {
    if (!range.empty)
      return {
        changes: [{ insert: token, from: range.from }, { insert: token, from: range.to }],
        effects: closeBracketEffect2.of(range.to + token.length),
        range: EditorSelection.range(range.anchor + token.length, range.head + token.length)
      };
    let pos = range.head, next = nextChar2(state.doc, pos), start;
    if (next == token) {
      if (nodeStart2(state, pos)) {
        return {
          changes: { insert: token + token, from: pos },
          effects: closeBracketEffect2.of(pos + token.length),
          range: EditorSelection.cursor(pos + token.length)
        };
      } else if (closedBracketAt2(state, pos)) {
        let isTriple = allowTriple && state.sliceDoc(pos, pos + token.length * 3) == token + token + token;
        let content = isTriple ? token + token + token : token;
        return {
          changes: { from: pos, to: pos + content.length, insert: content },
    ...
    } else if (allowTriple && state.sliceDoc(pos - 2 * token.length, pos) == token + token && (start = canStartStringAt2(state, pos - 2 * token.length, stringPrefixes)) > -1 && nodeStart2(state, start)) {
      return {
        changes: { insert: token + token + token + token, from: pos },
        effects: closeBracketEffect2.of(pos + token.length),
        range: EditorSelection.cursor(pos + token.length)
      ...
      if (canStartStringAt2(state, pos, stringPrefixes) > -1 && !probablyInString2(state, pos, token, stringPrefixes))
        return {
          changes: { insert: token + token, from: pos },
          effects: closeBracketEffect2.of(pos + token.length),
          range: EditorSelection.cursor(pos + token.length)
...
function nodeStart2(state, pos) {
  let tree = syntaxTree(state).resolveInner(pos + 1);
  return tree.parent && tree.from == pos;
}
function probablyInString2(state, pos, quoteToken, prefixes) {
  let node = syntaxTree(state).resolveInner(pos, -1);
  let maxPrefix = prefixes.reduce((m, p) => Math.max(m, p.length), 0);
  for (let i = 0; i < 5; i++) {
    let start = state.sliceDoc(node.from, Math.min(node.to, node.from + quoteToken.length + maxPrefix));
...
function canStartStringAt2(state, pos, prefixes) {
  let charCat = state.charCategorizer(pos);
  if (charCat(state.sliceDoc(pos - 1, pos)) != CharCategory.Word)
    return pos;
  for (let prefix of prefixes) {
...
var completionKeymap2 = [
  { key: "Ctrl-Space", run: startCompletion2 },
  { mac: "Alt-`", run: startCompletion2 },
  { key: "Escape", run: closeCompletion2 },
  { key: "ArrowDown", run: moveCompletionSelection2(true) },
  { key: "ArrowUp", run: moveCompletionSelection2(false) },
  { key: "PageDown", run: moveCompletionSelection2(true, "page") },
  { key: "PageUp", run: moveCompletionSelection2(false, "page") },
  { key: "Enter", run: acceptCompletion2 }
];
var completionKeymapExt2 = Prec.highest(keymap.computeN([completionConfig2], (state) => state.facet(completionConfig2).defaultKeymap ? [completionKeymap2] : []));
...
var android3 = typeof navigator == "object" && /Android\b/.test(navigator.userAgent);
var autoCloseTags = EditorView.inputHandler.of((view, from, to, text, defaultInsert) => {
  if ((android3 ? view.composing : view.compositionStarted) || view.state.readOnly || from != to || text != ">" && text != "/" || !javascriptLanguage.isActiveAt(view.state, from, -1))
    return false;
    ...
    props: [
      indentNodeProp.add({
        ...
        Document(context) {
          ...
          for (let cur3 = context.node; ; ) {
            let last = cur3.lastChild;
            if (!last || last.name != "Element" || last.to != cur3.to)
              break;
            endElt = cur3 = last;
          }
...
function getContext(node, doc) {
  let nodes = [], context = [];
  for (let cur3 = node; cur3; cur3 = cur3.parent) {
    if (cur3.name == "FencedCode")
      return context;
    if (cur3.name == "ListItem" || cur3.name == "Blockquote")
      nodes.push(cur3);
  }
...
function markdown(config3 = {}) {
  let { codeLanguages, defaultCodeLanguage, addKeymap = true, base: { parser: parser5 } = commonmarkLanguage, completeHTMLTags = true, htmlTagLanguage = htmlNoMatch } = config3;
